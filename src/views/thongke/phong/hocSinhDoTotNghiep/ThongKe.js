...
import { ThongKeHocSinhTotNghiepTheoTruong } from 'services/thongkeService';
...
const ThongKe = ({ isLoading }) => {
  ...
  useEffect(() => {
    const fetchDataDL = async () => {
      ...
      const response = await ThongKeHocSinhTotNghiepTheoTruong(params);
    ...
    series: [
      {
        ...
        data: data && data.thongKe && data.thongKe.length > 0 ? data.thongKe.map((item) => parseInt(item.tongSoHocSinhTotNghiep, 10)) : ''
      }
    ]
  };

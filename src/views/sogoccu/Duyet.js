...
import { duyetSoGocCu } from 'services/sogocService';

export default function Duyet({ donviOld }) {
  ...
  const hanldeGuiduyetAll = async () => {
    try {
      ...
      const response = await duyetSoGocCu(user.username, formData);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <MuiTypography variant="h4" gutterBottom m={1}>
        {`${t('Xác nhận duyệt sổ gốc của ')}${t('donvitruong.title')} [${donvi ? donvi.ten : ''}], ${t('danhmuc.title')} [${

...
import { khoaSoGocCu } from 'services/sogocService';

export default function Khoa({ donviOld }) {
  ...
  const hanldeGuiduyetAll = async () => {
    try {
      ...
      const response = await khoaSoGocCu(user.username, formData);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      <Grid item xs={12} mt={2}>
        <IconLock size={100} color="#F44336" />
      </Grid>
      <MuiTypography variant="h4" gutterBottom m={1}>
        {`${t('Xác nhận khóa sổ gốc của ')}${t('donvitruong.title')} [${donvi ? donvi.ten : ''}], ${t('danhmuc.title')} [${

...
import { hideTinTuc } from 'services/tintucService';
...
const HideTinTuc = () => {
  ...
  const handleDeleteClick = async () => {
    try {
      const response = await hideTinTuc(selectedTinTuc.id);
    ...
    <Grid container>
      ...
      <Grid item container xs={12} justifyContent={'center'} textAlign={'center'}>
        <MuiTypography variant="h4" gutterBottom>
          {`${t('Bạn có muốn ẩn Tin tức')} [${selectedTinTuc.tieuDe}]?`}
        </MuiTypography>
      </Grid>

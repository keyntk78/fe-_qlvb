...
const ResetPassword = () => {
  ...
  const handleResetClick = async () => {
    try {
      const userDeActive = await resetPasswordUser(selectedUser.userId);
    ...
    <Grid container textAlign={'center'}>
      ...
      <Grid item container xs={12} justifyContent={'center'}>
        <MuiTypography variant="h4" gutterBottom>
          {`${t('form.reset.warning')} ${t('label.user')} [${selectedUser.userName}]?`}
        </MuiTypography>
      </Grid>
      <Grid item container spacing={1} direction="row" justifyContent={'center'} my={2}>
        <Grid item>
          <YesButton handleClick={handleResetClick} />
        </Grid>
        <Grid item>
          <NoButton />
        </Grid>

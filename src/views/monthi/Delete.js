...
import { deleteMonthi } from 'services/monthiService';
import { setOpenPopup, setReloadData, showAlert } from 'store/actions';
import { selectedMonthiSelector } from 'store/selectors';
...
const DeleteMonthi = () => {
  ...
  const handleDeleteClick = async () => {
    try {
      const monthiDeleted = await deleteMonthi(monthi.id, user.username);
      if (monthiDeleted.isSuccess == false) {
        dispatch(showAlert(new Date().getTime().toString(), 'error', monthiDeleted.message.toString()));
      } else {
        dispatch(setOpenPopup(false));
        dispatch(setReloadData(true));
        dispatch(showAlert(new Date().getTime().toString(), 'success', monthiDeleted.message.toString()));
      }
    } catch (error) {

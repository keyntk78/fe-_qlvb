...
const ExportSoGoc = async (formik, pageState, danhmuc, donvi) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Sổ gốc');
    ...
    titleCell.value = 'SỔ GỐC CẤP BẰNG TỐT NGHIỆP TRUNG HỌC CƠ SỞ';
    ...
    namtn.value = `Năm tốt nghiệp: ${danhmuc.namThi}`;
    ...
    hinhthuchoc.value = `Hình thức học: ${danhmuc.hinhThucDaoTao}`;
    ...
    const date = formatDate(danhmuc.ngayCapBang);
    ...
    newRow.getCell(8).value = `${diaPhuong}, ${date}`;
    ...
    a.href = url;
    a.download = 'sogoc.xlsx';
    a.click();
};

export default ExportSoGoc;

...
import { duyetChinhSuaVBCC } from 'services/chinhsuavbccService';
...
function Duyet() {
  ...
  const hanldeDuyet = async () => {
    try {
      ...
      const response = await duyetChinhSuaVBCC(params);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <MuiTypography variant="h4" gutterBottom m={1}>
        {`${t('Xác nhận duyệt nội dung chỉnh sửa văn bằng của học sinh')} [${history?.hoTen || ''}]?`}
      </MuiTypography>

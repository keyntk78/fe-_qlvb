...
import { GetHocSinhDaDuyetByCCCD } from 'services/nguoihoctotnghiepService';
import { donviSelector, openPopupSelector, selectedCCCDSelector, userLoginSelector } from 'store/selectors';
...
const InGCN = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await GetHocSinhDaDuyetByCCCD(donvi.id, dataCccd);

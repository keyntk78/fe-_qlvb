...
import { KichHoatPhoiGoc } from 'services/phoigocService';
...
const ActivePhoiGoc = () => {
  ...
  const handleDeleteClick = async () => {
    try {
      const userActive = await KichHoatPhoiGoc(selectedPhoigoc.id);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <MuiTypography variant="h4" gutterBottom m={2}>
        {`${t('Bạn có muốn kích hoạt')} [${selectedPhoigoc.tenPhoi}]?`}
      </MuiTypography>
